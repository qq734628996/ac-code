...
void zero(int C) { ROF(i,C,V) dp[i] = max(dp[i], dp[i-C]+C); }
void comp(int C) { FOR(i,C,V) dp[i] = max(dp[i], dp[i-C]+C); }
void Multi(int C, int M)
{
    if (V <= C*M) { comp(C); return; }
