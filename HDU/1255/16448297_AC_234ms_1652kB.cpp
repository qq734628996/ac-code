...
void push_up(int x)
{
    ...
    if (t) tree[x].len[0] = Y[R] - Y[L-1];
    else if (L == R) tree[x].len[0] = 0;
    else tree[x].len[0] = tree[ls].len[0] + tree[rs].len[0];

    if (t >= 2) tree[x].len[1] = Y[R] - Y[L-1];
    else if (L == R) tree[x].len[1] = 0;
    else if (t == 1) tree[x].len[1] = tree[ls].len[0] + tree[rs].len[0];
    else tree[x].len[1] = tree[ls].len[1] + tree[rs].len[1];
}
void build(int x, int l, int r)
{
    tree[x].l = l, tree[x].r = r;
    tree[x].cover = 0;
    REP(i,2) tree[x].len[i] = 0;
    if (l<r) {
        int m = (l+r)>>1;
        build(ls,l,m);
        build(rs,m+1,r);
