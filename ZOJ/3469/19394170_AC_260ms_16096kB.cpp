...
int main()
{
    ...
    while (scanf("%lld%lld%lld", &n,&v,&X)==3) {
        ...
        FOR(len,1,n)FOR(i,1,n-len) {
            int j=i+len;
            d[i][j][0]=min(d[i+1][j][0]+(x[i+1]-x[i])*(S[n]-(S[j]-S[i])),
                           d[i+1][j][1]+(x[j]-x[i])*(S[n]-(S[j]-S[i])));
            d[i][j][1]=min(d[i][j-1][0]+(x[j]-x[i])*(S[n]-(S[j-1]-S[i-1])),
                           d[i][j-1][1]+(x[j]-x[j-1])*(S[n]-(S[j-1]-S[i-1])));
        }
